...
# normalization
train_cts <- train %>% select(-y) %>% select(cts_features)
...
trainn <- cbind(train_cts_normalized, train_categorical)
final_train <- trainn
...
## RF 0.78 test, 0.99 train, submission .78
## DT 74 test, 81 train
## SGB 79 test, 89 train, submission .72
...
model_RF <- train(y ~ ., data = final_train_balanced_over, method = "rf", trControl = fitControl, ntree=J, nodesize = min_size_of_terminal_nodes, tuneGrid = rf_grid)
...
final_train_balanced_overr <- final_train_balanced_over %>% select(-x37)
model_sgb <- train(y ~ ., data = data.frame(final_train_balanced_overr), method = "gbm", trControl = fitControl, tuneGrid = gbmGrid_forever, verbose=F)
#The final values used for the model were n.trees = 200, interaction.depth = 7, shrinkage = 0.2
