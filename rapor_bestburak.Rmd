...
Imbalanced data should be manipulated since prediction models are tent to predict target value as a majority of target result of imbalanced train data. There are many methods to overcome imbalanced data. Althought we use three different method, we had expect oversampling would give best results. The reasong of this, train and test data observations are few. Using undersampling may result information loss.This method mainly used on the data which has huge instance so reducing some instance would be beneficial for the implemantation of codes.

The accuracy of prediction model would be %75,74 and %71,14 for train and test data respectively, if there weren't made any manipulation to solve imbalancing problem..

Oversampling gives %80,98 balanced accuaracy for train and %80,26 balanced accuaracy for test.
Undersampling gives %83,54 balanced accuaracy for train and %77,94 balanced accuaracy for test.
Applying both oversampling and undersampling gives %80,35 balanced accuaracy for train and %77,67 balanced accuaracy for test.

Since highest and closest(to avoid overfitting) results obtain with oversampling, this method is choosen to solve imbalancing problem.
