...
up_train <- trainn %>% rename(Class=V1)
X_test_submission <- cbind(as.data.table(X_test_cts_scaled), as.data.table(X_test_submission_categorical))
X_uptrain <- up_train %>% select(-Class)


up_train <- up_train %>% select(-x37, -x57, -x59, -x26, -x49, -x46, -x43, -x60, -x29, -x55, -x31, -x18 )
...
y_train <- train %>% select(Class)
X_test <- test %>% select(-Class)
y_test <- test %>% select(Class)
...
set.seed(200)
RF <- train(Class~., data=train, method="ranger", trControl = fitControl, num.trees = 500, tuneGrid = tunegrid2, metric = "ROC",  importance = 'permutation')
RF

cm_test <- confusionMatrix(predict(RF, X_test, type = 'raw'), test$Class)
cm_train <- confusionMatrix(predict(RF, X_train, type = 'raw'), train$Class)
balanced_accuracy_test <- cm_test[["byClass"]][["Balanced Accuracy"]]
print(balanced_accuracy_test) #0.9519231
balanced_accuracy_train <- cm_train[["byClass"]][["Balanced Accuracy"]]
print(balanced_accuracy_train) #0.999601
...
tunegrid2 <- expand.grid(.mtry=c(3,5,10,15), .splitrule = "gini", .min.node.size = 5)
set.seed(200)
RF2 <- train(Class~., data=up_train, method="ranger", trControl = fitControl, num.trees = 500, tuneGrid = tunegrid2, metric = "ROC",  importance = 'permutation')

a <- RF2[["finalModel"]][["variable.importance"]] %>% as.data.frame()
colnames(a)[1] = "Importance"
a <- a %>% arrange(desc(Importance))

RF12
set.seed(1)
pred1 <- predict(RF2, X_test_submission, type = 'prob')  #testdata 
pred2 <- pred1 %>% select(b) %>% as.matrix() %>% as.double()


cm_uptrain <- confusionMatrix(predict(RF2, X_uptrain, type = 'raw'), up_train$Class)
balanced_accuracy_uptrain <- cm_uptrain[["byClass"]][["Balanced Accuracy"]]
