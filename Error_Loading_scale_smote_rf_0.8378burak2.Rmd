...
#qwe <- table(filtered_train1$y)




#length(filtered_train0$y=="a")
#count(filtered_train0, c("a", "b")) 
#print(table(filtered_train0$y))
#print(table(filtered_train1$y))


# "x4", "x12", "x13", "x15", "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x28", "x29", "x31", "x33", "x34", "x35", "x37", "x38", "x39", "x40", "x41", "x43", "x44", "x45", "x46", "x47", "x48", "x49", "x51", "x53", "x54", "x55", "x56", "x57", "x58", "x59", "x60"
####################

for (feature in c("x2", "x3", "x4", "x12", "x13", "x15", "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x28", "x29", "x31", "x33", "x34", "x35", "x37", "x38", "x39", "x40", "x41", "x43", "x44", "x45", "x46", "x47", "x48", "x49", "x51", "x53", "x54", "x55", "x56", "x57", "x58", "x59", "x60")) {


filtered_train0 <- filter(train_raw, train_raw[,feature]==0)
print(paste0(feature, " table " ,  table(filtered_train0$y) ))

a0_count <-as.data.frame((table(filtered_train0$y)))
a0_count1    <- filter(a0_count, a0_count$Var1=="a")
a0_count2 <-  as.numeric(a0_count1$Freq)

b0_count <-as.data.frame((table(filtered_train0$y)))
b0_count1    <- filter(b0_count, b0_count$Var1=="b")
b0_count2 <-  as.numeric(b0_count1$Freq)

print(paste0(feature, " ratio " ,  round(a0_count2/b0_count2, 3) ))
print(" ")

filtered_train1 <- filter(train_raw, train_raw[,feature]==1)
print(paste0(feature, " table " ,  table(filtered_train1$y) ))

a1_count <-as.data.frame((table(filtered_train1$y)))
a1_count1    <- filter(a1_count, a1_count$Var1=="a")
a1_count2 <-  as.numeric(a1_count1$Freq)

b1_count <-as.data.frame((table(filtered_train1$y)))
b1_count1    <- filter(b1_count, b1_count$Var1=="b")
b1_count2 <-  as.numeric(b1_count1$Freq)

print(paste0(feature, " ratio " ,  round(a1_count2/b1_count2, 3) ))
print(" ")
