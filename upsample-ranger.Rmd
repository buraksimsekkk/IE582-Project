...
train <- train_raw %>% select(-x50, -x52, -x57, -x37, -x19, -x26, -x49, -x29, -x47)
X_test_submission <- test_raw %>% select(-x50, -x52, -x57, -x37, -x19, -x26, -x49, -x29, -x47, -y)
set.seed(1)
trainnn <- upSample(x = train[, -ncol(train)], y = as.factor(train$y))   
...
set.seed(200)
RF <- train(Class~., data=trainnn, method="ranger", trControl = fitControl, tuneGrid = tunegrid)
RF
set.seed(200)
pred <- predict(RF, X_test_submission, type = 'raw')  #testdata 
pred
...
categorical_features <- c( "x2","x3","x4", "x10", "x12", "x13", "x15", "x16", "x17", "x18", "x20", "x21", "x22", "x23", "x24", "x25", "x28", "x31", "x33", "x34", "x35", "x38", "x39", "x40", "x41", "x43", "x44", "x45", "x46", "x48", "x51", "x53", "x54", "x55", "x56", "x58", "x59", "x60")
categorical_features_y <- c( "x2","x3", "x4", "x10","x12", "x13", "x15", "x16", "x17", "x18", "x20", "x21", "x22", "x23", "x24", "x25", "x28", "x31", "x33", "x34", "x35", "x38", "x39", "x40", "x41", "x43", "x44", "x45", "x46", "x48", "x51", "x53", "x54", "x55", "x56", "x58", "x59", "x60", "y")

library(BBmisc)

# scaling
train_cts_scaled <- trainnn %>% select(cts_features) %>% normalize() %>% as.data.frame()

X_test_cts_scaled <-X_test_submission %>% select(cts_features) %>% normalize() %>% as.data.frame()

# factorization
train_categorical <- trainnn %>% select(-Class) %>% select(categorical_features)
...
train_categorical <- trainnn %>% select(categorical_features)

X_test_submission_categorical <- X_test_submission %>% select(categorical_features)

# scaled and factored
trainn <- cbind(as.data.table(train_cts_scaled), as.data.table(train_categorical), as.data.table(trainnn$Class))
...
tunegrid <- expand.grid(.mtry=c(3,5,10,15), .splitrule = "gini", .min.node.size = 5)
set.seed(200)
RF <- train(V1~., data=trainnn, method="ranger", trControl = fitControl, tuneGrid = tunegrid)
RF
set.seed(200)
pred <- predict(RF, X_test_submission, type = 'raw')  #testdata 
pred
pred_tra <- predict(RF, trainnn, type = 'raw')  #testdata
cm_tra <- confusionMatrix(pred_tra, trainnn$V1)
balanced_accuracy_tra <- cm_tra[["byClass"]][["Balanced Accuracy"]]
print(balanced_accuracy_tra)


set.seed(150)
train_inds <- createDataPartition(y = 1:nrow(trainnn), p = 0.8, list = F)
test <- trainnn[-train_inds, ]
train <- trainnn[train_inds, ]

X_train <- train %>% select(-V1)
y_train <- train %>% select(V1)
X_test <- test %>% select(-V1)
y_test <- test %>% select(V1)

fitControl <- trainControl(method = "repeatedcv", number = 10, verboseIter = TRUE, classProbs = TRUE)
tunegrid <- expand.grid(.mtry=c(3, 5,10,15), .splitrule = "gini", .min.node.size = 5)

RF <- train(V1~., data=train, method="ranger", trControl = fitControl, tuneGrid = tunegrid, importance = 'impurity')
RF

pred <- predict(RF, X_test, type = 'raw')  #testdata 
cm_test <- confusionMatrix(pred, test$V1)
balanced_accuracy_test <- cm_test[["byClass"]][["Balanced Accuracy"]]
print(balanced_accuracy_test)

pred_train <- predict(RF, trainnn, type = 'raw')
cm_train <- confusionMatrix(pred_train, trainnn$V1)
...
```{r dataset_manip2}
#train <- train_raw %>% select(-x50, -x52, -x37, -x57, -x26, -x49, -x59, -x46)

train <- train_raw %>% select(-x50, -x52)
X_test_submission <- test_raw %>% select(-x50, -x52, -y)

set.seed(300)
trainnn <- upSample(x = train[, -59], y = as.factor(train$y))   
...
#####
#-x37, -x57, -x26, -x49, -x59, -x46

trainnn <- trainn %>% select(-x57, -x37)
...
fitControl <- trainControl(method = "repeatedcv", number = 10, verboseIter = TRUE, classProbs = TRUE, summaryFunction = twoClassSummary)
tunegrid <- expand.grid(.mtry=c(3, 5,10,15, 17, 21), .splitrule = "gini", .min.node.size = 5)

RF <- train(V1~., data=train, method="ranger", trControl = fitControl, tuneGrid = tunegrid, metric = "ROC", importance = 'permutation')
RF

a <- RF[["finalModel"]][["variable.importance"]] %>% as.data.frame()
...
pred_train <- predict(RF, train, type = 'raw')
cm_train <- confusionMatrix(pred_train, train$V1)
balanced_accuracy_train <- cm_train[["byClass"]][["Balanced Accuracy"]]
print(balanced_accuracy_train)
...
fitControl <- trainControl(method = "repeatedcv", number = 10, verboseIter = TRUE, classProbs = TRUE)
tunegrid <- expand.grid(.mtry=c(3, 5,10,15, 17, 21), .splitrule = "gini", .min.node.size = 5)
RF <- train(V1~., data=trainnn, method="ranger", trControl = fitControl, tuneGrid = tunegrid, importance = 'impurity')
RF

pred <- predict(RF, newdata=X_test_submission)
