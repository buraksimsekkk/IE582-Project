...
train <- train_raw %>% select(-x50, -x52, -x2, -x4, -x57, -x37, -x26)
X_test_submission <- test_raw %>% select(-x50, -x52, -x2, -x4, -x57, -x37, -x26, -y)
set.seed(500)
trainnn <- upSample(x = train[, -54], y = as.factor(train$y))   
...
categorical_features <- c("x3", "x47", "x12", "x13", "x15", "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x28", "x29", "x31", "x33", "x46", "x35", "x38", "x39", "x40", "x41", "x43", "x44", "x45", "x34", "x48", "x49", "x51", "x53", "x54", "x55", "x56", "x58", "x60", "x59")
categorical_features_y <- c("x3","x47", "x12", "x13", "x15", "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x28", "x29", "x31", "x33", "x46", "x35", "x38", "x39", "x40", "x41", "x43", "x44", "x45", "x34", "x48", "x49", "x51", "x53", "x54", "x55", "x56", "x58", "x59", "x60", "y")
...
set.seed(300)
RF11 <- train(V1~., data=up_train, method="ranger", trControl = fitControl, tuneGrid = tunegrid2,  metric="ROC", importance="permutation")
RF11
set.seed(300)
predtest11rf <- predict(RF11, X_test_submission, type = 'raw')  #testdata 
predtest11rf
pred_tra <- predict(RF11, up_train, type = 'raw')  #testdata
cm_tra <- confusionMatrix(pred_tra, up_train$V1)
balanced_accuracy_tra <- cm_tra[["byClass"]][["Balanced Accuracy"]]
print(balanced_accuracy_tra)


set.seed(300) #92.30 - 98.46
train_inds <- createDataPartition(y = 1:nrow(up_train), p = 0.8, list = F)
test <- up_train[-train_inds, ]
train <- up_train[train_inds, ]
...
RF11 <- train(V1~., data=train, method="ranger", trControl = fitControl, tuneGrid = tunegrid2, importance = 'permutation', metric="ROC")
RF11

a <- RF11[["finalModel"]][["variable.importance"]] %>% as.data.frame()
colnames(a)[1] = "Importance"
a <- a %>% arrange(desc(Importance))

pred <- predict(RF11, X_test, type = 'raw')  #testdata 
cm_test <- confusionMatrix(pred, test$V1)
balanced_accuracy_test <- cm_test[["byClass"]][["Balanced Accuracy"]]
print(balanced_accuracy_test)

pred_train <- predict(RF11, up_train, type = 'raw')
cm_train <- confusionMatrix(pred_train, up_train$V1)
