...
n_folds=10
fitControl<-trainControl(method = "repeatedcv", number = n_folds, repeats = 1, verboseIter = TRUE, classProbs = TRUE, search="random", savePred=TRUE, summaryFunction = twoClassSummary)   
rf_grid<-expand.grid(.mtry=c(3,5,10,15), .splitrule = "gini", .min.node.size = 5)

set.seed(50)
model_RF <- train(y ~ ., data = train, method = "ranger", trControl = fitControl, num.trees=J, tuneGrid = rf_grid, metric="ROC")
...
set.seed(50)
fitControl<-trainControl(method = "repeatedcv", number = n_folds, repeats = 1, verboseIter = TRUE, classProbs = TRUE, search="random", savePred=TRUE, summaryFunction = twoClassSummary)   
model_RF <- train(y ~ ., data = trainn, method = "ranger", trControl = fitControl, num.trees=J, tuneGrid = rf_grid, metric="ROC")
...
set.seed(200)
fitControl <- trainControl(method = "repeatedcv", number = 1, verboseIter = TRUE, classProbs = TRUE, summaryFunction = twoClassSummary)
tunegrid <- expand.grid(.mtry=c(3,5,10,15), .splitrule = "gini", .min.node.size = 5)
set.seed(200)
RF <- train(y~., data=train, method="ranger", trControl = fitControl, num.trees = 500, tuneGrid = tunegrid, metric = "ROC",  importance = 'permutation')
RF


cm_test <- confusionMatrix(predict(RF, X_test, type = 'raw'), test$y)
cm_train <- confusionMatrix(predict(RF, X_train, type = 'raw'), train$y)
